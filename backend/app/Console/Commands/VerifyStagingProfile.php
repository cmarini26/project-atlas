    ...
    public function handle(): int
    {
        ...
        $this->table(
            ...
            [
                ...
                ['onboarding', $profileRow !== null && $profileRow->isComplete() ? 'ok' : 'missing', $profileRow?->completed_at?->toDateTimeString() ?? 'not completed'],
                ['declared_channels', count($declaredTypes) >= 1 ? 'ok' : 'missing', implode(', ', $declaredTypes)],
                ['blog_channel', $blogChannel !== null ? 'ok' : 'missing', $blogChannel?->name ?? 'not found'],
                ['wordpress_connection', $wordpressCredentials !== null ? 'connected' : 'pending', $wordpressCredentials?->status ?? 'not connected'],
                ['email_connection', $emailCredentials !== null ? 'connected' : 'pending', $emailCredentials?->status ?? 'not connected'],
                ['audiences', $audienceCount === 4 ? 'ok' : 'mismatch', (string) $audienceCount],
                ['contacts', $contactCount === 4 ? 'ok' : 'mismatch', (string) $contactCount],
                ['discovery', $latestDiscovery !== null ? 'present' : 'pending', $latestDiscovery?->stage?->value ?? 'no discovery run'],
