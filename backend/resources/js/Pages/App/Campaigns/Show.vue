  ...
  <AppLayout>
    <div class="max-w-3xl">
      ...
      <div class="flex items-start gap-3 mb-6">
        <a href="/app/campaigns" class="mt-1 text-[--color-text-muted] hover:text-[--color-text-secondary] transition-colors duration-[--duration-fast]" aria-label="Back">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-sm text-[--color-text-muted] mt-1">Started {{ formatDate(campaign.created_at) }}</p>
        </div>
      </div>

      <!-- KPI snapshot -->
      <div v-if="kpi_snapshot" class="bg-[--color-surface-elevated] border border-[--color-border] rounded-xl p-5 mb-6">
        <h2 class="text-xs font-semibold text-[--color-text-muted] uppercase tracking-wide mb-3">Results</h2>
        <dl class="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div v-for="(value, key) in kpi_snapshot.actual_kpis" :key="key">
            ...
            <dd class="text-lg font-semibold text-[--color-text-primary] tabular-nums">{{ value }}</dd>
          </div>
        </dl>
      </div>
      ...
      <section v-if="content_assets.length > 0" class="mb-6">
        <h2 class="text-xs font-semibold text-[--color-text-muted] uppercase tracking-wide mb-3">Content</h2>
        <div class="space-y-3">
          <div
          ...
          >
            <div class="flex items-center gap-2 mb-2">
              ...
              <span v-if="asset.channel?.type" class="text-xs text-[--color-text-muted]">{{ asset.channel.type }}</span>
            </div>
            <h3 v-if="asset.title" class="text-sm font-semibold text-[--color-text-primary] mb-1">{{ asset.title }}</h3>
            <p class="text-sm text-[--color-text-secondary] whitespace-pre-line">{{ asset.body }}</p>
          </div>
        </div>
      </section>
      ...
      <!-- Executions -->
      <section>
        <h2 class="text-xs font-semibold text-[--color-text-muted] uppercase tracking-wide mb-3">Publishing</h2>
        ...
        <div v-else class="bg-[--color-surface-elevated] border border-[--color-border] rounded-xl divide-y divide-[--color-border]">
          ...
          >
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-[--color-text-primary] truncate">{{ execution.channel?.type ?? 'Unknown' }}</p>
