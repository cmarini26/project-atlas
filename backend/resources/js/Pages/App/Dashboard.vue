...
<template>
  <AppLayout>
    <div class="max-w-4xl">
      <h1 class="text-xl font-semibold text-[--color-text-primary] mb-6">Overview</h1>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-[--color-surface-elevated] border border-[--color-border] rounded-xl p-5">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-semibold text-[--color-text-primary]">Recent Campaigns</h2>
            <a href="/app/campaigns" class="text-xs text-[--color-text-link] hover:underline">View all</a>
          </div>
          ...
          <div v-if="recent_campaigns.length > 0" class="space-y-3">
            ...
            >
              <a :href="`/app/campaigns/${campaign.id}`" class="text-sm text-[--color-text-secondary] hover:text-[--color-text-primary] truncate flex-1 mr-3">
                {{ campaign.title }}
              </a>
      ...
      <div class="bg-[--color-surface-elevated] border border-[--color-border] rounded-xl p-5">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-sm font-semibold text-[--color-text-primary]">Recent Publishing Activity</h2>
          <a href="/app/publishing" class="text-xs text-[--color-text-link] hover:underline">View all</a>
        </div>
