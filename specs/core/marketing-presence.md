...
readonly class BusinessBrain
{
    public function __construct(
        ...
        public Collection $marketingPresence,   // NEW — Collection<int, MarketingChannel>
    ) {}
}
```

**Unfiltered, like `recentCampaigns` — not pre-filtered, like `activeFacts`.** `BusinessBrainService::assemble()` should load *all* of a company's `MarketingChannel` rows (all `status` values, including `inactive`), the same way `recentCampaigns` includes campaigns of every status. This is a deliberate choice: the Opportunity Engine's boundary rule ("do not recommend inactive channels unless explicitly justified," Section 9) requires the engine to be able to *see* inactive channels in order to deliberately exclude them — a pre-filtered collection that silently hides them would make that exclusion accidental rather than intentional, and would make it impossible to ever justify an exception.

**Cache invalidation.** Following the existing pattern for `FactExtracted` and `KnowledgeSynthesized` (`AppServiceProvider::boot()`), a new event — `MarketingPresenceUpdated` — is fired by `MarketingPresenceService` after any create, update, or delete of a `MarketingChannel`, and a listener calls `BusinessBrainService::invalidate($event->companyId)`. One coarse event, not one per CRUD verb: the only consumer (cache invalidation) doesn't care what changed, only that it changed. Follow Founding Principle 7 — add more granular events later only if a second consumer needs to distinguish create from update from delete.
...
**How prompts use it.** Any `Analyst` receiving the `BusinessBrain` (rationale generation, campaign preparation, content generation) can now reference `$brain->marketingPresence`. The immediate, concrete use in this milestone is read-only context — e.g., `CampaignPreparationAnalyst`'s prompt can be told "this business's primary channels are Instagram (weekly) and email (biweekly); they also occasionally exhibit at events" even before any of those channels can be published to. This makes campaign strategy and audience descriptions (`CampaignBlueprint.audience`, `core_message`) more accurate immediately, with zero publishing work. Filtering to "active" or "primary" channels for a specific prompt's context window is the prompt-builder's responsibility (e.g. `$brain->marketingPresence->where('status', 'active')`), not the Business Brain's.
