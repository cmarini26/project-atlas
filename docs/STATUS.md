...
|-------------------|--------|-------|
| Specifications    | ✅ Complete | Domain model, architecture, database, AI, MVP workflow, analytics engine, and learning engine all defined. `specs/core/marketing-presence.md` — Milestone 11 domain spec, approved; **Phases 1–6 (domain model, service layer, onboarding, Settings UI, Business Brain integration, channel selection) now implemented**. |
| Implementation    | ✅ Customer dashboard complete | All 10 milestones delivered. Full customer-facing Vue 3 + Inertia.js dashboard live. Milestone 11 (Marketing Presence) Phases 1–6 shipped — see [Milestone-11-Phase-1-Review.md](reviews/Milestone-11-Phase-1-Review.md) through [Milestone-11-Phase-6-Review.md](reviews/Milestone-11-Phase-6-Review.md). Phases 7–8 (Recommendation UI, remaining tests) not started. |
| Tests             | ✅ Strong | 826 tests (824 passing, 2 Redis skipped) + 18 Vitest tests; PHPStan level 8 — 0 errors; Pint clean. Latest: Milestone 11 Phase 6 — Marketing-Presence-aware Decision Engine channel selection, 17 new tests. |
...
**Overall:** Milestone 10 complete + onboarding pipeline fixed (Phase 1–9) + P0 product-polish tier shipped + P1 Customer Trust & Navigation slice shipped (approval confirmation dialog, safety-tested company switcher, persistent layout + `Link` sweep, toast primitive — see [P1-Customer-Trust-Navigation-Review.md](reviews/P1-Customer-Trust-Navigation-Review.md)) + Channel Publishing Reality audit complete (see [Channel-Publishing-Reality-Audit.md](reviews/Channel-Publishing-Reality-Audit.md)) + Milestone 11 specified and planned (see [marketing-presence.md](../specs/core/marketing-presence.md) and [Milestone-11-Marketing-Presence.md](plans/Milestone-11-Marketing-Presence.md)) + **Milestone 11 Phases 1–6 implemented** (see [Milestone-11-Phase-1-Review.md](reviews/Milestone-11-Phase-1-Review.md), [Milestone-11-Phase-2-Review.md](reviews/Milestone-11-Phase-2-Review.md), [Milestone-11-Phase-3-Review.md](reviews/Milestone-11-Phase-3-Review.md), [Milestone-11-Phase-4-Review.md](reviews/Milestone-11-Phase-4-Review.md), [Milestone-11-Phase-5-Review.md](reviews/Milestone-11-Phase-5-Review.md), and [Milestone-11-Phase-6-Review.md](reviews/Milestone-11-Phase-6-Review.md)). The `marketing_channels` table, `App\Models\MarketingChannel`, five new backed enums, and `MarketingChannelFactory` exist — declaring a marketing channel is representable in the database with zero API connection required. `App\Services\MarketingPresence\MarketingPresenceService` provides the full CRUD lifecycle; `MarketingChannelCapabilityResolver` derives a channel's domain-level lifecycle stage. The onboarding wizard has a fourth, final step declaring channels with zero metadata. A Settings sub-page (`/app/settings/marketing-presence`) lets a user view/add/edit/disable declared channels with capability badges. `App\Domain\BusinessBrain\BusinessBrain` carries a synthesized `MarketingPresenceSummary` (never raw rows), kept fresh via the `MarketingPresenceUpdated` event invalidating the Business Brain's per-process memo. **New in Phase 6:** `App\Services\Decision\MarketingChannelSelector` makes `DecisionEngine::evaluate()` Marketing-Presence-aware when resolving `Decision.channel_ids` — preferring `primary`-linked channels, excluding `inactive`/`planned`-linked ones (with a safety bypass so a company is never left with zero candidates), and reporting declared-but-unlinked channels as draft-only content targets, all via a `MarketingChannelSelection` value object carried on the (non-persisted) `DecisionContext` — no `Decision` or `CampaignBlueprint` schema change. A `MarketingChannel` never becomes executable without a linked `Channel` row, and no `Channel` row is ever created automatically. No Opportunity detection changes, no publishing changes were made — Phases 7–8 (Recommendation UI, remaining tests) not started. Remaining P1 items (email notifications, Sentry, AI usage persistence, icon/Button/FormField primitives, a first real channel publisher) and all P2 items are tracked in [Product-Polish-Audit.md](reviews/Product-Polish-Audit.md). 826 tests (824 passing, 2 Redis skipped) + 18 Vitest tests. PHPStan level 8 — 0 errors. Pint clean.
