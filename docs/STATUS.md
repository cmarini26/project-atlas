...
|-------------------|--------|-------|
| Specifications    | ✅ Complete | Domain model, architecture, database, AI, MVP workflow, analytics engine, and learning engine all defined. `specs/core/marketing-presence.md` — Milestone 11 domain spec, approved; **Phases 1–2 (domain model, service layer) now implemented**. |
| Implementation    | ✅ Customer dashboard complete | All 10 milestones delivered. Full customer-facing Vue 3 + Inertia.js dashboard live. Milestone 11 (Marketing Presence) Phase 1 (domain model) and Phase 2 (service layer) shipped — see [Milestone-11-Phase-1-Review.md](reviews/Milestone-11-Phase-1-Review.md) and [Milestone-11-Phase-2-Review.md](reviews/Milestone-11-Phase-2-Review.md). Phases 3–8 (onboarding, Settings UI, Business Brain, Opportunity Engine, Recommendation UI, remaining tests) not started. |
| Tests             | ✅ Strong | 760 tests (758 passing, 2 Redis skipped) + 13 Vitest tests; PHPStan level 8 — 0 errors; Pint clean. Latest: Milestone 11 Phase 2 — `MarketingPresenceService` + `MarketingChannelCapabilityResolver`, 45 new tests. |
...
**Overall:** Milestone 10 complete + onboarding pipeline fixed (Phase 1–9) + P0 product-polish tier shipped + P1 Customer Trust & Navigation slice shipped (approval confirmation dialog, safety-tested company switcher, persistent layout + `Link` sweep, toast primitive — see [P1-Customer-Trust-Navigation-Review.md](reviews/P1-Customer-Trust-Navigation-Review.md)) + Channel Publishing Reality audit complete (see [Channel-Publishing-Reality-Audit.md](reviews/Channel-Publishing-Reality-Audit.md)) + Milestone 11 specified and planned (see [marketing-presence.md](../specs/core/marketing-presence.md) and [Milestone-11-Marketing-Presence.md](plans/Milestone-11-Marketing-Presence.md)) + **Milestone 11 Phase 1 (Domain Model) implemented** (see [Milestone-11-Phase-1-Review.md](reviews/Milestone-11-Phase-1-Review.md)) + **Milestone 11 Phase 2 (Service Layer) implemented** (see [Milestone-11-Phase-2-Review.md](reviews/Milestone-11-Phase-2-Review.md)). The `marketing_channels` table, `App\Models\MarketingChannel` (with `belongsTo(Company)`/`belongsTo(Channel)`, `active()`/`primary()`/`connected()` scopes, structural validation rules), five new backed enums (`MarketingChannelType`, `Status`, `Importance`, `Objective`, `PostingFrequency`), and `MarketingChannelFactory` now exist — declaring a marketing channel (Instagram, print, events, and everything else a business owner would name) is representable in the database with zero API connection required, exactly as specified. On top of that, `App\Services\MarketingPresence\MarketingPresenceService` now provides the full CRUD lifecycle (`declare`/`update`/`setStatus`/`disable`/`reactivate`/`link`), per-type `suggestedDefaults()`, a non-persisting `suggestChannels()` (website integration + existing `Channel` rows), and a soft `wouldDuplicate()` check; `App\Services\MarketingPresence\MarketingChannelCapabilityResolver` is the single place that derives a channel's domain-level lifecycle stage (Declared/Connected/PublishingEnabled/AnalyticsEnabled); a new `MarketingPresenceUpdated` event fires on every mutation but has no listener yet. No onboarding changes, no Settings UI, no Business Brain integration, no Opportunity Engine changes, and no publishing changes were made — those remain Phases 3–8, not started. Remaining P1 items (email notifications, Sentry, AI usage persistence, icon/Button/FormField primitives, a first real channel publisher) and all P2 items are tracked in [Product-Polish-Audit.md](reviews/Product-Polish-Audit.md). 760 tests (758 passing, 2 Redis skipped) + 13 Vitest tests. PHPStan level 8 — 0 errors. Pint clean.
