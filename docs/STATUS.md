...
Replaces Phase 1's "Start Discovery" placeholder button with real, source-agnostic orchestration: pressing it now inspects every declared Marketing Asset, dispatches the existing connector/observation pipeline for whichever ones can currently be observed, and tracks progress as a pure observability layer alongside it — Discovery orchestrates the existing Observation → Fact → Knowledge → Opportunity → Recommendation pipeline, it does not replace or duplicate any part of it. No changes to Business Brain, Marketing Health, the Opportunity Engine, the Decision Engine, or the Connector architecture itself.

**Source-agnostic by construction, not by convention:** the new `App\Services\Observatory\Connectors\Contracts\AutoDiscoverableConnector` interface (implemented so far only by `WebsiteConnector`) lets a connector *declare* which `MarketingChannelType` it can auto-discover and how to build an `Integration` config for it — `ConnectorRegistry::autoDiscoverableFor()` answers "what can be observed?" generically. `App\Services\Discovery\DiscoveryPlanner::planFor()` asks exactly two source-agnostic questions per declared asset: is it already linked to a connected `Integration` (resync it), or does any registered connector declare it auto-discoverable (create one)? Neither the planner nor `App\Services\Discovery\BusinessDiscoveryService` (the orchestrator) contains a single reference to "website," "Instagram," or any other platform name — adding a future no-auth-capable connector (e.g. a public Google Business lookup) requires zero changes to either.

**Closed the real linkage gap the Phase 1 design docs already identified:** `marketing_channels.integration_id` (new nullable FK) and `MarketingPresenceService::linkIntegration()` let a declared asset become `is_connected: true` via an `Integration` (an observation source), mirroring `link()`'s existing `Channel` (publishing-destination) path exactly. `SettingsController::connectInstagram()` now calls it, so an Instagram asset declared during onboarding correctly shows as connected once the user connects it for real from Settings — and Discovery then resyncs it on any future run, exactly like Website.

**Resilience is structural, not a special case:** `BusinessDiscoveryService::attempt()` wraps each `SyncIntegration::dispatch()` in a try/catch — one unreachable website can never abort the rest of the run. `DiscoveryConnectorAttempt` rows (new table) represent `pending`/`running`/`succeeded`/`failed`/`skipped_no_credentials` independently per asset; a new `IntegrationSyncFailed` event (mirroring the existing `IntegrationSyncCompleted`) and the `UpdateDiscoveryConnectorAttempt` listener keep them current. `DiscoveryRun.stage` (`discovering`/`analyzing`/`understanding`/`recommending`/`completed`/`completed_with_errors`) is never incrementally mutated — `BusinessDiscoveryService::refreshStage()` recomputes it from scratch from real persisted state (attempt statuses, processed Observations, synthesized Knowledge, pending Recommendations) every time it's read, so it can never drift out of sync with reality.

**Idempotent by the same mechanism that makes it resilient:** rerunning Discovery for a company whose Website integration already exists simply resyncs that same `Integration` (the planner's "already connected" branch) rather than creating a second one — no duplicate `MarketingChannel` or `Integration` rows across any number of runs, verified explicitly.

The old single-`Integration`-scoped `OnboardingStatusController`/`Status.vue` were replaced: the API now aggregates `BusinessDiscoveryService::progressFor()` across a company's whole `DiscoveryRun`, and the UI shows the four coarse stages (Discover → Analyze → Understand → Recommend), per-asset connector status underneath Discover (✓ succeeded, ✕ failed with its error, ○ not attempted/not connected/not supported yet), and — once complete — a real completion summary (succeeded assets, facts created, opportunities found, recommendations generated, all read from persisted counts, never fabricated) with a manual link to the first recommendation rather than an instant redirect.

16 new PHP tests (`BusinessDiscoveryServiceTest` covering website-only, website+already-connected-asset reuse, one-connector-fails resilience, all-fail → `completed_with_errors`, declared-but-unsupported assets, idempotent reruns, all six stage transitions computed directly against persisted fixtures, and summary generation; `DiscoveryPlannerTest` for the reuse/create/null decision in isolation) plus 3 new `MarketingPresenceServiceTest` cases (`linkIntegration()`, tenant-mismatch guard, `scopeConnected()` across both linkage paths) and 4 new Vitest tests for the rewritten `Status.vue`. Both new migrations (`marketing_channels.integration_id`, `discovery_runs`, `discovery_connector_attempts`) were verified against real local PostgreSQL. One real cross-database migration issue was found and fixed: adding a foreign key to an existing table via `Schema::table()` forces SQLite to rebuild the table from its Doctrine-introspected column list, silently dropping the raw `CHECK` constraints backing `marketing_channels`' `enum()` columns — the migration now skips the FK on SQLite (test environment only; Postgres/production get the real, enforced FK) rather than lose that coverage.
...
**2026-08-28** — Stripe billing epic (CM-73–79) complete — none of it existed on `main` despite CM-73 being marked Done. **CM-73:** `App\Billing\Contracts\BillingProvider` (the only seam product code touches: ensureCustomer / createSubscriptionCheckout / createBillingPortalSession / parseWebhookEvent / fetchSubscription) with value objects, `BillingException`/`WebhookVerificationException`, a `StripeBillingProvider` (wraps `stripe/stripe-php`, never leaks vendor errors), a `FakeBillingProvider` (in-memory, local/testing), and explicit `BILLING_DRIVER` selection. **CM-76:** `billing_profiles` (one per company: Stripe linkage, `subscription_status`, period, `beta_access_override`) + `BillingProfile::grantsAccess()` + `BillingProfileService` (idempotent single writer of billing truth). **CM-74:** `BillingCheckoutService` + `POST /app/settings/billing/checkout` — ensures/links a Stripe customer, opens a subscription Checkout Session (`client_reference_id` = company id), owner/admin only. **CM-75:** `POST /api/stripe/webhook` — signature-verified (400 bad sig, 500→retry, 200 handled), `stripe_webhook_events` idempotency + support log, `StripeWebhookProcessor` mirrors `checkout.session.completed` + `customer.subscription.*` onto billing state. **CM-77:** `App/Settings/Billing.vue` + controller — honest status/period/cancel state, Subscribe / Manage-billing (Stripe portal), post-checkout banners, never overclaims. **CM-78:** `BillingAccess` + `RequireBillingAccess` middleware (`billing` alias) gating recommendation approval; `BILLING_GATE_ENABLED=false` beta-safe default (restricts nothing); per-company `beta_access_override` + `php artisan billing:beta-access`. **CM-79:** HTTP-mocked `StripeBillingProvider` request-mapping tests via a fake `Stripe\HttpClient`, and `docs/operations/Stripe-Billing-Runbook.md` (setup, secrets, webhook config, `stripe listen`, gating, reconciliation). ~70 billing tests. Test-mode only — no live keys; `BILLING_DRIVER=fake` in `phpunit.xml`.

**2026-08-28** — CM-89 (onboarding marketing-presence disclosure) complete — 3 stories. **CM-91:** `App\Domain\Onboarding\OnboardingAssetTypes` is the shared source of truth for the 10 Marketing Assets cards plus each type's post-onboarding `integrationRequirement` (`none` | `handle` | `oauth` | `manual`) and a one-line summary; mirrored in `resources/js/lib/onboardingAssets.ts` with a parity test guarding drift; `OnboardingController` now derives its asset-card allow-list from it. **CM-92:** enabled step-4 chips whose requirement is `oauth`/`handle` show an inline "Needs setup" badge (text, not colour-only); step-4 subtitle reworded to say selections are recorded now and connected later; non-blocking. **CM-93:** `PendingChannelSetup::forCompany()` computes — live — the declared-but-unconnected channels (`handle` unsatisfied until a URL is entered, `oauth` until `is_connected`); the dashboard renders a dismissible `ChannelSetupReminder` banner listing them with plain-language next steps and links to Marketing Presence. Dismissal persists per-user (`users.channel_setup_reminder_dismissed_at`, new `POST /app/channel-setup/dismiss`, `auth.user.has_dismissed_channel_setup` shared prop). Chosen as a dashboard element over a 6th onboarding step per the ticket's open question. ~15 new PHP tests + 2 Vitest specs; `onboarding-to-recommendation.spec.ts` extended for the multi-channel path.

**2026-08-28** — CM-90 (prompt-first custom campaign creation) complete — 4 stories. **CM-94:** inverted the custom-campaign contract — `source_asset_ids` is now `nullable`, the objective prompt is the required input, and `title` is derived from the prompt when omitted. `CustomCampaignService::compose()` handles the zero-asset path (Business Brain as sole grounding) and fails with an actionable validation message when there is neither an asset nor an established Digital Twin. **CM-96:** new swappable `ImageProvider` abstraction under `app/AI/Images/` mirroring the `AiProvider` pattern — `ImageGenerationRequest`/`GeneratedImage`/`ImageGenerationException` (provider-agnostic, `retryable` flag), `OpenAiImageProvider` (`gpt-image-1` at `quality=low`, bounded retries, structured per-generation logging), `FakeImageProvider` (placeholder PNG, no network, local+testing only), `ImageStorage` (public disk, `campaign-images/{companyId}/…`). Selected via `config/ai.php` → `image.provider`; docs in `docs/technical/AI.md`. **CM-97:** `GenerateCampaignImagery` queued job (queue `ai`) wired into compose — builds a Business-Brain-grounded prompt via `CampaignImagePrompt` (never a raw-objective passthrough), generates one image, records provider/model/cost on the new `campaign_image_generations` table (also the rolling-window cap ledger). `ImageGenerationCap` enforces a per-company limit before the provider is called; a breach or any failure marks the generation `failed` with an actionable message and never blocks campaign creation or approval. Imagery is automatic when no user assets are supplied, opt-in (`generate_imagery`) when they are. **CM-95:** the campaign Create page is rebuilt prompt-first — the description textarea is the primary control, the asset picker moved into a collapsed-by-default `<details>` section, the hard EmptyState block is gone, and the submit button works on a valid prompt alone. Recommendation review UI surfaces pending / ready / failed imagery alongside copy. ~30 new PHP tests + 5 new Vitest tests.

**2026-08-27** — CM-84 (Ollama resilience, health checks, observability) done — part of the CM-80 local-LLM epic. `OllamaAiProvider` now retries transient failures (connection errors, HTTP 5xx) with bounded backoff (4 attempts max) and raises a typed `LocalAiException` hierarchy — `LocalAiUnavailableException` (transient, `implements RetryableAiException`), `LocalAiModelMissingException`, `LocalAiOutOfMemoryException`, `LocalAiInvalidResponseException` — each carrying a machine-readable `category` and operator-actionable `guidance`. `ProcessObservation` catches `RetryableAiException` (hosted overload *or* local unavailable → observation `retrying`, not `failed`) and fails fast on permanent `LocalAiException` (no wasted retries hammering the local model). New `LocalAiHealthService` probes `/api/tags` for reachability + configured-model presence; wired into `/api/ready` (only when `ai.provider=ollama`) and exposed as `php artisan ai:local:health [--json]`. Structured per-call logging (provider, model, latency, tokens, `failure_category`) with no prompt bodies outside `APP_DEBUG`. ~40 new/updated PHP tests. PHPStan level 8 clean, Pint clean.

**2026-08-27** — CM-87 (local LLM operations runbook) done — part of the CM-80 local-LLM epic. New `docs/operations/Local-LLM-Runbook.md`: host profile (Mac mini 24 GB, `qwen3:14b` ~13 GB peak, 8K context, single `ai`-queue worker), install + `brew services` management, model lifecycle (`pull`/`show`/`ps`/`stop`/`rm`, keep-alive/unload), the two localhost-binding guarantees + how to verify the listening socket, the health checks (`ai:local:health`, `/api/ready`), enable/disable via `AI_FACT_EXTRACTION_PROVIDER` with no-code-change rollback, the `LocalAiException` failure matrix with operator actions, a troubleshooting section, upgrade + eval-gated model-bump procedure, and disk cleanup. Opens by stating local-model output is subject to the same schema validation + human approval as hosted output. Linked from `docs/technical/AI.md`. Docs only.

**2026-08-27** — CM-86 (Ollama vs Anthropic evaluation harness) done — part of the CM-80 local-LLM epic. `php artisan ai:eval:fact-extraction` runs the **real** `FactExtractionPrompt` (semantics unchanged) through each named provider over synthetic cases in `eval/fact-extraction/cases/` (3 seeded: comic auction house, exotic car dealer, coffee roaster — no customer data) and writes a machine-readable JSON report to `storage/app/eval/` (git-ignored). Per provider it reports schema-valid rate, key-set precision/recall/F1, a value-substring accuracy, an unsupported-claims-per-case proxy for hallucination, latency, and token counts, with provider failures bucketed by `LocalAiException` category. Acceptance thresholds live in `config('ai.eval.thresholds')`; the command exits non-zero when the gate provider (default `ollama`) misses one — Anthropic is measured as baseline, never gated. Every report records `git_sha`, model, and Ollama `settings` (context, think); quantization is recorded by hand from the model tag. `FactExtractionEvaluator` + command, 8 tests, `docs/technical/Local-LLM-Evaluation.md`. Runs against live providers are an operator step. Stacked on CM-84 + CM-85.

**2026-08-27** — CM-85 (Ollama fact-extraction pilot) done — part of the CM-80 local-LLM epic. Added task-level AI provider routing: `AiProviderFactory` resolves a provider by name (extracted from `AppServiceProvider`, same explicit selection + env gates), and `config('ai.task_providers.fact_extraction')` / `AI_FACT_EXTRACTION_PROVIDER` routes **only** `WebsiteAnalyst` (the sole AI-backed fact-extraction path) to a chosen provider via a contextual binding — every other task stays on `AI_PROVIDER`. Unset the env var to restore the prior path with no code change. Routed facts still go through `StructuredResponseParser` + schema validation and keep `observation_id` provenance. Active routing is logged once at boot. 4 new tests; `docs/technical/AI.md` updated. End-to-end verification against a real local model on the Mac mini is an operator step (see CM-87 runbook).
**2026-08-27** — CM-90 (prompt-first custom campaign creation) complete — 4 stories. **CM-94:** inverted the custom-campaign contract — `source_asset_ids` is now `nullable`, the objective prompt is the required input, and `title` is derived from the prompt when omitted. `CustomCampaignService::compose()` handles the zero-asset path (Business Brain as sole grounding) and fails with an actionable validation message when there is neither an asset nor an established Digital Twin. **CM-96:** new swappable `ImageProvider` abstraction under `app/AI/Images/` mirroring the `AiProvider` pattern — `ImageGenerationRequest`/`GeneratedImage`/`ImageGenerationException` (provider-agnostic, `retryable` flag), `OpenAiImageProvider` (`gpt-image-1` at `quality=low`, bounded retries, structured per-generation logging), `FakeImageProvider` (placeholder PNG, no network, local+testing only), `ImageStorage` (public disk, `campaign-images/{companyId}/…`). Selected via `config/ai.php` → `image.provider`; docs in `docs/technical/AI.md`. **CM-97:** `GenerateCampaignImagery` queued job (queue `ai`) wired into compose — builds a Business-Brain-grounded prompt via `CampaignImagePrompt` (never a raw-objective passthrough), generates one image, records provider/model/cost on the new `campaign_image_generations` table (also the rolling-window cap ledger). `ImageGenerationCap` enforces a per-company limit before the provider is called; a breach or any failure marks the generation `failed` with an actionable message and never blocks campaign creation or approval. Imagery is automatic when no user assets are supplied, opt-in (`generate_imagery`) when they are. **CM-95:** the campaign Create page is rebuilt prompt-first — the description textarea is the primary control, the asset picker moved into a collapsed-by-default `<details>` section, the hard EmptyState block is gone, and the submit button works on a valid prompt alone. Recommendation review UI surfaces pending / ready / failed imagery alongside copy. ~30 new PHP tests + 5 new Vitest tests; full suite green except 5 pre-existing `AiProviderConfigurationTest` subprocess failures unrelated to this work.
