...
|-------------------|--------|-------|
| Specifications    | ✅ Complete | Domain model, architecture, database, AI, MVP workflow, analytics engine, and learning engine all defined. `specs/core/marketing-presence.md` — Milestone 11 domain spec, approved; **Phases 1–5 (domain model, service layer, onboarding, Settings UI, Business Brain integration) now implemented**. |
| Implementation    | ✅ Customer dashboard complete | All 10 milestones delivered. Full customer-facing Vue 3 + Inertia.js dashboard live. Milestone 11 (Marketing Presence) Phase 1 (domain model), Phase 2 (service layer), Phase 3 (onboarding), Phase 4 (Settings UI), and Phase 5 (Business Brain integration) shipped — see [Milestone-11-Phase-1-Review.md](reviews/Milestone-11-Phase-1-Review.md), [Milestone-11-Phase-2-Review.md](reviews/Milestone-11-Phase-2-Review.md), [Milestone-11-Phase-3-Review.md](reviews/Milestone-11-Phase-3-Review.md), [Milestone-11-Phase-4-Review.md](reviews/Milestone-11-Phase-4-Review.md), and [Milestone-11-Phase-5-Review.md](reviews/Milestone-11-Phase-5-Review.md). Phases 6–8 (Opportunity Engine channel-selection, Recommendation UI, remaining tests) not started. |
| Tests             | ✅ Strong | 809 tests (807 passing, 2 Redis skipped) + 18 Vitest tests; PHPStan level 8 — 0 errors; Pint clean. Latest: Milestone 11 Phase 5 — Business Brain Marketing Presence synthesis + cache invalidation, 25 new tests. |
...
**Overall:** Milestone 10 complete + onboarding pipeline fixed (Phase 1–9) + P0 product-polish tier shipped + P1 Customer Trust & Navigation slice shipped (approval confirmation dialog, safety-tested company switcher, persistent layout + `Link` sweep, toast primitive — see [P1-Customer-Trust-Navigation-Review.md](reviews/P1-Customer-Trust-Navigation-Review.md)) + Channel Publishing Reality audit complete (see [Channel-Publishing-Reality-Audit.md](reviews/Channel-Publishing-Reality-Audit.md)) + Milestone 11 specified and planned (see [marketing-presence.md](../specs/core/marketing-presence.md) and [Milestone-11-Marketing-Presence.md](plans/Milestone-11-Marketing-Presence.md)) + **Milestone 11 Phase 1 (Domain Model) implemented** (see [Milestone-11-Phase-1-Review.md](reviews/Milestone-11-Phase-1-Review.md)) + **Milestone 11 Phase 2 (Service Layer) implemented** (see [Milestone-11-Phase-2-Review.md](reviews/Milestone-11-Phase-2-Review.md)) + **Milestone 11 Phase 3 (Onboarding) implemented** (see [Milestone-11-Phase-3-Review.md](reviews/Milestone-11-Phase-3-Review.md)) + **Milestone 11 Phase 4 (Settings UI) implemented** (see [Milestone-11-Phase-4-Review.md](reviews/Milestone-11-Phase-4-Review.md)) + **Milestone 11 Phase 5 (Business Brain Integration) implemented** (see [Milestone-11-Phase-5-Review.md](reviews/Milestone-11-Phase-5-Review.md)). The `marketing_channels` table, `App\Models\MarketingChannel` (with `belongsTo(Company)`/`belongsTo(Channel)`, `active()`/`primary()`/`connected()` scopes, structural validation rules), five new backed enums (`MarketingChannelType`, `Status`, `Importance`, `Objective`, `PostingFrequency`), and `MarketingChannelFactory` now exist — declaring a marketing channel (Instagram, print, events, and everything else a business owner would name) is representable in the database with zero API connection required, exactly as specified. `App\Services\MarketingPresence\MarketingPresenceService` provides the full CRUD lifecycle (`declare`/`update`/`setStatus`/`disable`/`reactivate`/`link`), per-type `suggestedDefaults()`, a non-persisting `suggestChannels()` (website integration + existing `Channel` rows), and a soft `wouldDuplicate()` check; `App\Services\MarketingPresence\MarketingChannelCapabilityResolver` is the single place that derives a channel's domain-level lifecycle stage (Declared/Connected/PublishingEnabled/AnalyticsEnabled). The onboarding wizard has a fourth, final data-entry step — "Where do your customers find you?" — a checkbox-only, zero-metadata channel checklist (Website preselected) that declares one unlinked, unconnected `MarketingChannel` per selection before the pipeline status page is reached. A Settings sub-page (`/app/settings/marketing-presence`) lets a user view, add, edit (status/importance/objectives), and disable/reactivate declared channels, each showing a capability badge resolved server-side. `App\Domain\BusinessBrain\BusinessBrain` now carries a synthesized `MarketingPresenceSummary` (primary/secondary/inactive channel names, primary objectives, and one composed natural-language sentence) rather than raw `MarketingChannel` rows — `App\Services\Brain\MarketingPresenceSynthesizer` builds it deterministically (no AI call), and the previously-inert `MarketingPresenceUpdated` event now invalidates the Business Brain's per-process memo on every mutation, mirroring the existing `FactExtracted`/`KnowledgeSynthesized` pattern with no synchronous rebuild and no new queue. No Opportunity Engine channel-selection changes, no Decision Engine changes, and no publishing changes were made — those remain Phases 6–8, not started. Remaining P1 items (email notifications, Sentry, AI usage persistence, icon/Button/FormField primitives, a first real channel publisher) and all P2 items are tracked in [Product-Polish-Audit.md](reviews/Product-Polish-Audit.md). 809 tests (807 passing, 2 Redis skipped) + 18 Vitest tests. PHPStan level 8 — 0 errors. Pint clean.
