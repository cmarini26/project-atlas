...
- **Capability badge caveat:** `MarketingChannelType` still has **no** `sms` case, so the generic capability-badge system has no per-company `supports_publishing` override path for SMS yet. The Settings surface is the accurate source of truth for whether a company's Twilio connection is live; generic badge rendering elsewhere still lacks a first-class SMS-specific connected state. `channelCapability.ts`'s global fallback was `coming_later` ("no way to create... for anyone") until 2026-08-05, which was wrong even at the narrow, per-destination scope described above — fixed to `not_configured` ("real connect flow exists, this company hasn't used it"), matching the same category `facebook`/`instagram` already use.
