...
## Addendum — 2026-07-16: email is now real; the capability matrix is the new canonical source

Production-readiness gap plan, Phase 0/1 close-out. Since the 2026-07-15 addendum above, three more things happened, closing two of that addendum's own "did not do" items and adding a fourth real channel:

1. **`Publishing.vue`, `Dashboard.vue`, and `Campaigns/Show.vue` now all pass real per-company `linked-marketing-channel` data** to `ChannelCapabilityBadge` (`PublishingController::index()`, `DashboardController::index()`, `CampaignController::show()` each build a `$linkedMarketingChannelsByChannelId` map and attach it to every execution/content-asset row). All three pages can now correctly show "Connected" for a genuinely live Meta or Email channel — the one gap the previous addendum explicitly left open.
2. **Email is now a fully real, fully closed-loop channel**: `SettingsController::connectEmail()` (ping-verified connect), a real company-scoped contact/audience model (`EmailAudienceService`), real multi-recipient sending (`EmailPublisher::publishToAudience()`), a real analytics provider with the canonical cross-channel keys fixed (`PostmarkAnalyticsProvider`), and three email-specific `LearningService` signals. Email is no longer "Logs internally only" — see `docs/product/Channel-Capability-Matrix.md` for the full per-stage breakdown.
3. **Two UI truth bugs were found and fixed while writing the capability matrix**: `ApproveActions.vue`'s per-asset approval-effect line unconditionally said "logged internally, not yet sent live" even for a channel resolved as `'connected'` — a self-contradicting statement at the single most safety-critical moment in the product (approval). `Publishing.vue`'s page description made the same blanket claim ("Atlas doesn't publish to live external channels yet") directly above a list of rows whose own badges could say "Connected." Both now branch on the real per-channel/per-company capability instead of asserting a single global state. `Dashboard.vue`/`Campaigns/Show.vue`'s empty-state copy was corrected the same way.

### This audit document is now superseded as the canonical source

**`docs/product/Channel-Capability-Matrix.md` is the new canonical per-channel truth table**, covering all six lifecycle stages (Observe/Draft/Approve/Execute/Measure/Learn), not just execution. This document remains useful as a narrative history of how that truth was discovered and progressively closed, but any future capability change should update the matrix first and this document only as historical record.
